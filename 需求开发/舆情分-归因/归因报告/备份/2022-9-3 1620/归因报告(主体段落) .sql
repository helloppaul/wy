...
(
	select distinct 
		...
		score,second_score,third_score,comprehensive_score,label_hit
	from RMP_ALERT_COMPREHS_SCORE_TEMP a 
...
(
	...
	(
		...
		from app_ehzh.rsk_rmp_warncntr_opnwrn_intp_sentiself_feapct_intf a   --app_ehzh_train.featpct_senti_self
		join (select max(end_dt) as max_end_dt from app_ehzh.rsk_rmp_warncntr_opnwrn_intp_sentiself_feapct_intf) b
			on a.end_dt=b.max_end_dt
		join (select * from corp_chg where source_code='FI') chg
			on cast(a.corp_code as string) = chg.source_id
		join app_ehzh.rsk_rmp_warncntr_opnwrn_feat_sentiself_val_intf c   --app_ehzh_train.featvalue_senti_self
...
(
	...
	)A group by batch_dt,corp_id,corp_nm,score_dt,feat_cnt,feat_cnt_0p9
),
second_two_msg as 
(
	select 
...
(
	...
	group by com.corp_id,com.corp_nm,com.score_dt,com.label_hit,rsk.msg_id,rsk.case_type,rsk.case_type_ii,rsk.signal_type
),
second_three as 
(
	select 
	...
	(
		select 
			*,
			...
			concat(case_type_ii,'(',importance_map,')') corp_rsk_msg
		from 
		(
			select distinct
				corp_id,
...
select 
	...
	CASE
		when score=0 THEN   --主体部分没有
			''
		when score>0 and score>rel_score_summ then 
			concat('	其中，主要',sentence_2)
		when score>0 and score<rel_score_summ then 
			concat('	其次，',sentence_2)
	end as sentence_2
from second_msg
