...
-- create table if not exists pth_rmp.rmp_warning_score_detail_test stored as parquet like pth_rmp.rmp_warning_score_detail;

create table pth_rmp.rmp_warning_score_detail_test_impala stored as parquet as
--—————————————————————————————————————————————————————— 配置表 ————————————————————————————————————————————————————————————————————————————————--
with
feat_CFG as  --特征手工配置表
(
...
(
    select *
    from pth_rmp.rmp_warn_feature_contrib_test
),
warn_contribution_ratio as 
(
    select * 
    from pth_rmp.rmp_warn_contribution_ratio_test
),
warn_feature_contrib_res3 as 
(
    select *
    from pth_rmp.rmp_warn_feature_contrib_res3_test
),
-- 特征原始值 --
warn_feature_value_with_median_res as 
(
    select *
    from pth_rmp.rmp_warn_feature_value_with_median_res_test
),
-- 特征打分卡 --
warn_score_card as 
(
    select *
    from pth_rmp.rmp_warn_score_card_test
),
--—————————————————————————————————————————————————————— 应用层 ————————————————————————————————————————————————————————————————————————————————--
-- 结果集 --
...
------------------------------------以上部分为临时表-------------------------------------------------------------------
-- insert into pth_rmp.rmp_warning_score_detail_test partition(etl_date=${ETL_DATE})
select 
...
--     concat(corp_id,'_',MD5(concat(batch_dt,dimension,type,sub_model_name,idx_name))) as sid_kw,  --hive
--     a.*
-- from pth_rmp.rmp_warning_score_detail_test_impala a
