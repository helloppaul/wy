...
--impala执行 可通过参数END_DT,控制具体跑批批次
-- 特征原始值+中位数计算 -- 
drop table if exists pth_rmp.rmp_warn_feature_value_with_median_res_test;
create table pth_rmp.rmp_warn_feature_value_with_median_res_test as 
--—————————————————————————————————————————————————————— 基本信息 ————————————————————————————————————————————————————————————————————————————————--
with
...
(
    ...
    (
        ...
        (
            ...
            where end_dt = (${END_DT})
              OR  to_date(end_dt) = to_date(date_add(from_unixtime(unix_timestamp(cast(${ETL_DATE} as string),'yyyyMMdd')),-1))
