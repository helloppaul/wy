...
(
	...
	(	
		...
		(
			...
			FROM 
			(
				select *,ROW_NUMBER() over(partition by COMPANY_ID,NOTICE_DT order by 1) as rm,count(*) over(partition by COMPANY_ID,NOTICE_DT) as case_no_cnt
				from 
				(
					select 
						COMPANY_ID,
