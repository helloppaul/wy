...
(
    select issuecompcode,secode,`symbol`, bondname,maturitydate
    from hds.t_ods_fic_ic_tq_bd_basicinfo 
    where isvalid='1'
),
t_ods_fic_hb_tq_bd_stockchg_ as --债券存量变动表(增量更新)
(
    select secode,natural_dt,changedamt,aftchangedamt
...
(
    select distinct
        ...
        c.aftchangedamt/10000 as chg_amt  --变动后金额(亿元)
...
select 
    ...
    -- md5(concat(corp_id,lead_underwriter)) as sid_kw,  -- hive
    corp_id,
    natural_dt,
    recent_maturity_date,
