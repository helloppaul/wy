...
deal_featvalue as(
	...
	(
		...
		from (	select distinct batch_dt,corp_code,end_dt,feature_name,feature_value
...
(
	...
	(
		...
		(
			...
			from pth_rmp.RMP_COMPANY_CORE_REL_ a 
		...
		(
			...
			(
				select 
					...
					row_number() over(partition by a.corp_id,score_dt,a.relation_id order by a.r_importance asc) as min_rm
