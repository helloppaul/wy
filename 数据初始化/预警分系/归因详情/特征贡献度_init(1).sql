-- 特征贡献度-初始化 warn_contribution_ratio + warn_feature_contrib + warn_feature_contrib_res3 -- 

-- part1 2min--
create table pth_rmp.warn_contribution_ratio as 
with
...
-- part2 4 min-- 
create table pth_rmp.warn_feature_contrib as 
with 
corp_chg as  --带有 城投/产业判断和国标一级行业 的特殊corp_chg
(
	select distinct a.corp_id,b.corp_name,b.credit_code,a.source_id,a.source_code
...
-- part3 2min--
create table pth_rmp.warn_feature_contrib_res3 as 
with
--—————————————————————————————————————————————————————— 接口层 ————————————————————————————————————————————————————————————————————————————————--
-- 时间限制开关 --
timeLimit_switch as 
...
select * 
from warn_feature_contrib_res3
;